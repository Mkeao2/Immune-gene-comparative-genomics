...
2. Match the longest isoform gene names with gene symbol, transcript ID and protein ID (Find_longest_isoforms_1.py).

3. Run the following to get a list of genes with all immune associated GO terms (import_2.py). 
...
4. Extract only FBgns and GO columns from this list (file 3).
5. Merge files 1 and 3 to get a list with columns FBgn_ID, FBtr_ID, FBpp_ID and Gene_Symbol. 
